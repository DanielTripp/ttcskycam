...
def get_polylines_from_files():
	r = []
	for track_filename in sorted(f for f in os.listdir('tracks') if f.endswith('.json')):
		r.append(get_polyline_from_file(os.path.join('tracks', track_filename)))
	return r
...
def get_polyline_from_file(filename_):
	try:
		...
		printerr('error with %s' % filename_)
		raise
