...
if 0:
	...
	for tolerance in (0, 1, 2):
		...
		lat = startlat
		while lat < endlat:
			print '%.5f, %.5f   -> %d' % (lat, lon, routes.latlon_to_mofr(latlng(lat, lon), 'king', tolerance_=tolerance))
			lat += 0.00001
if 0:
	...
	for tolerance in (0, 1, 2):
		...
		for ratio in frange(0.0, 1.0, 0.01):
			...
			print '%.5f, %.5f   -> %d' % (lat, lon, routes.latlon_to_mofr(latlng(lat, lon), 'dundas', tolerance_=tolerance))
			lat += 0.00001
...
if 0:
	...
	for i, pt in enumerate(king_pts):
		print '%d: ( %.5f, %.5f ) -> %d' % (i, pt[0], pt[1], routes.latlon_to_mofr(latlng(pt[0], pt[1]), 'king'))
	...
	for i, pt in enumerate(dundas_pts):
		print '%d: ( %.5f, %.5f ) -> %d' % (i, pt[0], pt[1], routes.latlon_to_mofr(latlng(pt[0], pt[1]), 'dundas'))
	...
	for i, pt in enumerate(queen_pts):
		print '%d: ( %.5f, %.5f ) -> %d' % (i, pt[0], pt[1], routes.latlon_to_mofr(latlng(pt[0], pt[1]), 'queen'))
print '--- get_traffics:'
